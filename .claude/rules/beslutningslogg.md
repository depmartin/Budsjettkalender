...
- Sist fullført: **Fase 2-koding STARTET. Steg A (kildeabstraksjon) implementert** i `backend/kilder` (`Aarshjul.Kilder`): `IKilde` (`OppdagAsync`/`HentAsync`), `OppdagResultat`/`Oppdagutfall` (uttrykker utfall: `FantDokumenter`/`IngenDokumenter`/`KlarteIkkeParse`), `HentResultat`/`Hentutfall`, `Dokumentreferanse`. Koblet inn i `Aarshjul.slnx` + Web/Tests. 27/27 tester grønt på .NET 10. Forut: Fase 1 (steg 1–8 + kodegjennomgang av PR #1, fem funn rettet) og et designintervju (se beslutningen over) som låste rammevalg og forfininger.
- Neste steg (Fase 2 — fortsett fra Steg B):
  1. Les `CLAUDE.md` + denne loggen + `arkitektur.md` (særlig «Miljøoppsett» — .NET 10 SDK må installeres i ferskt miljø — og «Fase 1 — implementert»).
  2. Bekreft at `dotnet test backend/Aarshjul.slnx` er grønt (27/27) før ny kode.
  3. **Steg B — `RegjeringenKilde.OppdagAsync()`:** les arkivsiden, parse tabellrader (Nummer/Tittel/Dato/Status), utled PDF-URL. Parse-feil → `KlarteIkkeParse` (ikke tom liste). Krever HTML-parser (velg i steget) og nett-tilgang til regjeringen.no (sjekk nettverkspolicyen).
  4. Resterende fase 2-leveranser (oppdatert med intervjuforfiningene): dedup mot `BehandletDokument` + **forkastet-liste** og **auto endringsforslag ved ny versjon** (Steg C); totrinns filtrering (Steg D); `IDatouttrekk`-abstraksjon + auto-forkast-regel (Steg E); **godkjenningskø** (Steg F, gjenbruk `Synlighetsfilter` + `FristskrivingTjeneste`-mønsteret); redigeringsskjema (G); brukerforslag + «mine forslag» (H); endringsforslag (I); `Varsel` (J); Word-utskrift (K); bakgrunnsjobb `BackgroundService` + manuell trigger (L).
