...
- **Miljøoppsett (ferskt miljø):** .NET 10 SDK + `dotnet-ef` 10.0.9 må installeres (se arkitektur.md «Miljøoppsett»); bekreft `dotnet test backend/Aarshjul.slnx` grønt (128/128) før ny kode. Kjør demo lokalt: `dotnet run --project backend/Aarshjul.Web --launch-profile demo`, så `/demo` for å velge persona.
