...
## Status nå
- Aktiv fase: **Fase 1 MERGET (PR #1). Fase 2 — offline-delene kodet i PR #5** (Steg A/D/F/G/H/I/J/K + `IDatouttrekk`-forberedelse; 94 tester grønt). Gjenstående Fase 2 (Steg B/C/E/L) er blokkert til egress mot `www.regjeringen.no` åpnes. **Fase 3-planlegging AVSLUTTET 2026-06-19** (`fase3-plan.md`), og Fase 3 er **neste kodetrinn** — avhengighetene (Steg F/G) er nå på plass. **Ny sesjon for Fase 3:** merge PR #5 til `main` først, start ny gren fra `main`, og legg fram en Fase 3-plan (CLAUDE.md krever plan + godkjenning før kode i ny fase) før Steg A i `fase3-plan.md`.
