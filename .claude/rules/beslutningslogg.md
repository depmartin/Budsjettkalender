...
- Sist fullført: **Fase 2 Steg M — manuell dokumentopplasting KODET 2026-07-21** (`/admin/last-opp`, kun admin): PDF-tekst (PdfPig) → `IDatouttrekk` (`HeuristiskDatouttrekk`, offline) → `Totrinnsfilter` → `Forslag`+`UttrekksBevis` i den vanlige køen — samme uttrekk som automatisk, uten kildeleddet. Låste opp den forberedte `IDatouttrekk`-bruken. **Tilhørende fiks:** interaktivitet var ikke aktivert i Blazor-appen (ingen `@rendermode`); satt `InteractiveServer` på `Routes`/`HeadOutlet` i `App.razor` — nå virker alle knapper/handlinger. 140 tester grønt. Se beslutningsoppføringen «Manuell dokumentopplasting» nedenfor. (Forut: demo-modus + CSS-/asset-fiks; Fase 3; PR #6–#10.)
...
- **Miljøoppsett (ferskt miljø):** .NET 10 SDK + `dotnet-ef` 10.0.9 må installeres (se arkitektur.md «Miljøoppsett»); bekreft `dotnet test backend/Aarshjul.slnx` grønt (140/140) før ny kode. Kjør demo lokalt: `dotnet run --project backend/Aarshjul.Web --launch-profile demo`, så `/demo` for å velge persona.
