...
- Sist fullført: **Fase 3 (mal og generering) — Steg A–G, 125/125 tester grønt på .NET 10**, åpen som **PR #6** (`claude/cool-clarke-vm4uoi` → `main`; CI «Bygg og test» grønt; denne sesjonen abonnerer på PR-aktivitet). Se beslutningsoppføringen «Fase 3 … kodet» (2026-06-19) for detaljer.
- **Miljøoppsett (ferskt miljø):** .NET 10 SDK + `dotnet-ef` 10.0.9 må installeres (se arkitektur.md «Miljøoppsett»); bekreft `dotnet test backend/Aarshjul.slnx` grønt (125/125) før ny kode.
