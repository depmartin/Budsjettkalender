...
- Sist fullført: **`fra_dag`-modus på `RelativUkedag` KODET 2026-07-21** — «første ukedag på/etter en gitt dag i måneden» (f.eks. «mandagen i uken 20.–26. juli» = `{ "maaned":7,"ukedag":"man","fra_dag":20 }`). `Datoberegning.FoersteUkedagFraOgMed`, valgfritt `fra_dag` på `RelativUkedagParametre`, brukt i `Genereringsberegning` (bakoverkompatibelt: uten `fra_dag` gjelder `uke`/n-te forekomst). 151 tester grønt. (Forut: Fase 2 Steg M dokumentopplasting; demo-modus + CSS-/asset-fiks; Fase 3; PR #6–#13.)
