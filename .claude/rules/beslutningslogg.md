...
- Sist fullført: **Fase 2-koding pågår.** Ferdig så langt: **Steg A** (kildeabstraksjon i `backend/kilder`/`Aarshjul.Kilder`: `IKilde`, `OppdagResultat`/`Oppdagutfall` = `FantDokumenter`/`IngenDokumenter`/`KlarteIkkeParse`, `HentResultat`/`Hentutfall`, `Dokumentreferanse`); **modelltillegg + EF-migrasjon `Fase2Innhenting`** (`BehandletDokument.UttrekksForsoek`/`SisteForsoek`; `BehandletStatus` utvidet med `HentingFeilet`/`FeiletFlagget`; `FristStatus.Forkastet`; ny entitet `InnhentingsStatus` for liveness); **Steg D** (totrinns filtrering: `Loepmonstre` + `Totrinnsfilter.Klassifiser`). 37/37 tester grønt på .NET 10. `dotnet-ef` 10.0.9 installeres i miljøet (global tool). Forut: Fase 1, PR #3/#4 merget, og et designintervju som låste rammevalg/forfininger (se beslutningene over).
...
- Neste steg (Fase 2):
  ...
  2. Bekreft at `dotnet test backend/Aarshjul.slnx` er grønt (37/37) før ny kode.
  3. **Steg B (blokkert til egress):** `RegjeringenKilde.OppdagAsync()` — les arkivsiden, parse tabellrader, utled PDF-URL (`…/arlige/{aar}/r-{nr}-{aar}.pdf` + eldre fallback). Bruk `HttpClient` med ekte User-Agent (regjeringen.no 403-er ellers). Parse-feil → `KlarteIkkeParse`.
  4. **Offline-delbare neste:** `IDatouttrekk`-abstraksjon + deterministiske usikkerhetsregler + auto-forkast-regel (lav konfidens OG ingen dato) — ren logikk, testbar uten live modell (Steg E-forberedelse). Deretter **godkjenningskø** (Steg F, Blazor admin; gjenbruk `Synlighetsfilter` + `FristskrivingTjeneste`-mønsteret) som ikke krever live kilde.
  5. Resterende: dedup + forkastet-liste + auto endringsforslag (C); redigeringsskjema (G); brukerforslag + «mine forslag» (H); endringsforslag (I); `Varsel` (J); Word-utskrift (K); bakgrunnsjobb `BackgroundService` + manuell trigger (L). Deretter Fase 3 (kun etter eksplisitt beskjed fra bruker).
