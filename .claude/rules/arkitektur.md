...
> STATUS: Fase 1 (fundament) ferdig. **Fase 2 — offline-delbare steg kodet** (A/B/D/F/G/H/I/J/K + `IDatouttrekk`). **Fase 3 (mal og generering) KODET 2026-06-19 (Steg A–G): `Datoberegning` (FastDato/NteUkedag/Virkedagjuster) + `Valgaar`, typede regelparametre + `Regelparser`, `Genereringsberegning` (anker-kjeder/sirkularitet/tentativ-arv/valgår), `GenereringsTjeneste` (videreføring av synlighet), konfig-drevet `Synlighetsregel`, `Maltjeneste`, og admin-flatene `/admin/generer` + `/admin/mal`. EF-migrasjon `Fase3Generering` (la til `Gjentaksregel.Tittel`).** **Lokal demo-modus (miljø `Demo`): SQLite + dev-innlogging (`/demo`) + demo-seed (`Demodata`), uten Azure SQL/Entra — nå på `main` (PR #7).**
>
> **NYTT på gren `claude/project-status-next-steps-24073n` (PR #8), 155 tester grønt på .NET 10:**
> (a) **Steg B — `RegjeringenKilde` + `RegjeringenParser`** (AngleSharp), verifisert offline mot ekte markup (Wayback-fixtur), fordi `www.regjeringen.no` ligger bak **Cloudflare «challenge»** (403 på ikke-nettleserklienter) selv om egress er åpnet — arkitekturfunn som også rammer en fremtidig Azure-jobb (Steg L).
> (b) **Manuell opplasting (admin, `/admin/last-opp`)** — hele innhentingspipelinen (`PdfPigTekst` → dedup (SHA-256, kilde=`regjeringen`) → totrinnsfilter → datouttrekk → `Forslag` i køen); testinngang nå + varig reserveløsning. Minimal dedup (ingen EF-migrasjon); full Steg C venter på live.
> (c) **Multi-frist-uttrekk:** `IDatouttrekk.TrekkUtAsync` returnerer `IReadOnlyList<Uttrekksresultat>` — **én `Forslag` per frist** (samme `DokumentId`, arvet løp/kategori), fordi ett rundskriv har mange frister.
> (d) **Ekte datouttrekk mot Claude** (`ClaudeDatouttrekk` + testbar `ClaudeUttrekk`) bak `IDatouttrekk`; `Program.cs` velger ekte Claude når nøkkel finnes (`Datouttrekk:ApiNokkel`/`ANTHROPIC_API_KEY`), ellers `FakeDatouttrekk`.
> (e) **Blazor-interaktivitet fikset** (gjelder også prod): global `@rendermode InteractiveServer` på `App.razor`, `@using Microsoft.AspNetCore.Components.Web` i rot-`_Imports.razor`, `UseStaticWebAssets()` i Demo.
>
> Gjenstående Fase 2 (Steg C dedup/versjonsmatching, E live datouttrekk, L bakgrunnsjobb) er **blokkert** av Cloudflare på `www.regjeringen.no` (IT-avklaring). **MERK:** Fase 1 + Fase 2-offline + Fase 3 + demo er på `main`; (a)–(e) over ligger på gren `claude/project-status-next-steps-24073n` (PR #8, ikke merget). Bekreftet stack: **.NET 10 (LTS) / ASP.NET Core + EF Core, Azure SQL, Blazor Web App (Interactive Server), Azure App Service**; bakgrunnsjobb = `BackgroundService` i web-hosten; datouttrekk bak `IDatouttrekk` (default Claude API, lokasjon IT-avklart). Se beslutningsloggen for full fremdrift og åpne forhold.
...
- Test (alle): `dotnet test backend/Aarshjul.slnx` — 155 tester (synlighet, datoberegning, innlegging, claims, HTTP-integrasjon, godkjenningskø, brukerforslag, Word-eksport, generering/valgår/anker-kjeder, demo-seed, Steg B-parsing, opplastingspipeline, Claude-uttrekkstolkning).
- Kjør appen lokalt: `dotnet run --project backend/Aarshjul.Web` (krever DB + Entra-konfig; se appsettings).
- **Kjør lokal demo (uten Azure SQL/Entra):** `ASPNETCORE_ENVIRONMENT=Demo dotnet run --project backend/Aarshjul.Web`. Miljøet `Demo` bruker SQLite (`aarshjul-demo.db`, git-ignorert), `EnsureCreated` + demo-seed (`Demodata`), og en dev-innlogging på `/demo` der man velger persona (cookie-auth, ingen Entra). Autorisering/synlighet kjører nøyaktig som i produksjon — kun identitetskilden er byttet. Demo-kode rører ikke produksjonsstien.
