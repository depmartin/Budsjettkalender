...
> STATUS: Fase 1 (fundament) ferdig. **Fase 2 — offline-delbare steg kodet** (A/D/F/G/H/I/J/K + `IDatouttrekk`-forberedelse). **Fase 3 (mal og generering) KODET 2026-06-19 (Steg A–G): `Datoberegning` (FastDato/NteUkedag/Virkedagjuster) + `Valgaar`, typede regelparametre + `Regelparser`, `Genereringsberegning` (anker-kjeder/sirkularitet/tentativ-arv/valgår), `GenereringsTjeneste` (videreføring av synlighet), konfig-drevet `Synlighetsregel`, `Maltjeneste`, og admin-flatene `/admin/generer` + `/admin/mal`. EF-migrasjon `Fase3Generering` (la til `Gjentaksregel.Tittel`).** **Lokal demo-modus (miljø `Demo`): SQLite + dev-innlogging (`/demo`) + demo-seed (`Demodata`), uten Azure SQL/Entra (drives av flagget `Demo=true` i Development-miljøet).** **Fase 2 Steg M — manuell dokumentopplasting KODET 2026-07-21 (`/admin/last-opp`, kun admin): PDF-tekst (PdfPig) → `IDatouttrekk` (`HeuristiskDatouttrekk`, offline) → `Totrinnsfilter` → `Forslag`+`UttrekksBevis` i køen; samme uttrekk som automatisk. Interaktivitet aktivert (`@rendermode="InteractiveServer"` i `App.razor`).** **UI-/uttrekksfikser 2026-07-22 (PR #15, gren `claude/ui-og-uttrekk-fikser`):** opplastingsuttrekket tar med hele setningen (setningssegmentering + hevet tegngrense); den separate «Nå»-/landingsvisningen er fjernet (Kalender er inngang på `/`); filtrene fikset (rot-`_Imports.razor` manglet `@using Microsoft.AspNetCore.Components.Web`).** **Fire funksjonsendringer 2026-07-22 (gren `claude/admin-eksport-mal-fikser`):** admin-rolleskille (ny policy `ErBidragsyter`; «Mine forslag»/«Varsler» kun bidragsyter; admin redigerer frister direkte via `Fristkort` «Rediger» → `/admin/frist/{id}`); eksport/import av frister som JSON (`IFristDatautveksling`, `/admin/data`, `/api/eksport/frister-json`, erstatt-alt-import); begrunnelse ved avvisning kun for bidragsyter-forslag; og fiks av at `RelativUkedag`-regler med `fra_dag` ble stille droppet ved generering når løpet kolliderte (nå én frist per regel).** **Milepæl-dropdown + mal-eksport 2026-07-22 (gren `claude/mal-milepael-og-eksport`):** `RelativTilMilepael` i malregel-skjemaet velger milepæl fra nedtrekksliste (JSON bygges automatisk); eksport/import av alle årsmaler som JSON (`IMalDatautveksling`, `/api/eksport/maler-json`, `/admin/data`).** **Kalendereksport (.ics) + valgfritt klokkeslett 2026-07-30 (gren `claude/remaining-tasks-function-update-tpf641`):** `IKalenderEksport`/`IcsEksportTjeneste` bygger en iCalendar-fil (RFC 5545) med hele det synlighetsfiltrerte utvalget (gruppe/«alt» + periode) som VEVENT-er til import i Outlook; endepunkt `/api/eksport/ics` (admin), knapp på `/admin/eksport` ved siden av Word (delt utvalgsledd). Frister fikk et valgfritt `Klokkeslett` (`TimeOnly?`, standard heldags) — EF-migrasjon `KalendereksportKlokkeslett`; kalenderhendelser blir heldags, eller tidfestet (norsk tid → UTC) når klokkeslett er satt. (Merget til main via PR #18.)** **Delbar kalender-feed + auto-admin via Entra-gruppe 2026-07-30 (gren `claude/remaining-tasks-function-update-tpf641`):** (1) `Kalenderabonnement` + `IKalenderabonnement`/`KalenderabonnementTjeneste` + anonymt feed-endepunkt `GET /kalender/feed/{token}.ics` (tokenet er autoriseringen; server-side synlighetsfiltrert per gruppe; ukjent/avskrudd → 404) + admin-flate `/admin/abonnement` («Del kalender»): auto-oppdaterende abonnementslenke per gruppe til Outlook/SharePoint, av/på + slett; manuell .ics-nedlasting beholdt. EF-migrasjon `Kalenderabonnement`. (2) Medlemskapsstyrt admin via konfig `Administratortilgang` (Entra-gruppe SBR) i `BrukeroppslagTjeneste`; `Bruker.AdminViaEntra`; seedet admin beholdes. EF-migrasjon `AdminViaEntra`. 184 tester grønt på .NET 10. **NB: demo bruker EnsureCreated — slett `aarshjul-demo.db` ved pull for at nye kolonner/tabeller skal opprettes.** **Internkalender for SBR KODET 2026-08-03 (Fase 4, trinn 1–3, på `main`):** SBR-intern arbeidsliste atskilt fra fristene, bak ny policy `ErSbr` (= administrator i v1). Domenemodell `InternRunde`/`InterntGjoeremaal`/`GjoeremaalAnsvarlig` + `GjoeremaalRegel`/`RegelRundetype`/`RegelAnsvarlig`, `Runder`-domenelogikk (månedsspenn/årsforskyvning/rundeposisjon). Tjenester `IInternkalender`/`InternkalenderTjeneste` (runder, gjøremål, tidfesting→sorteringsdag med «venter på ankerdato», avhuking med hvem/når, «mine» på tvers av runder, generering, synk) og `IGjoeremaalRegler`/`GjoeremaalRegelTjeneste` (regel-CRUD). Flater under `Components/Pages/Internkalender`. Konkret runde = snapshot av reglene; «Synkroniser» gir forslag (legg til/oppdater/fjern) som aldri rører avhukede/manuelt endrede gjøremål. EF-migrasjon `Internkalender`. 216 tester grønt (+32). Se `fase4-plan.md`. **NB: demo bruker EnsureCreated — slett `aarshjul-demo.db` ved pull for at de nye tabellene skal opprettes.** Gjenstående Fase 2 (Steg B oppdagelse, C dedup/versjonsmatching, E live datouttrekk, L bakgrunnsjobb) er **blokkert** til `www.regjeringen.no` åpnes i egress-allowlisten. **MERK:** alt kodet så langt er på `main` — Fase 1, Fase 2-offline (inkl. Steg M dokumentopplasting), Fase 3, demo-modus og fiksene for CSS/asset-servering og Blazor-interaktivitet (PR #6–#11 merget). Bekreftet stack: **.NET 10 (LTS) / ASP.NET Core + EF Core, Azure SQL, Blazor Web App (Interactive Server), Azure App Service**; bakgrunnsjobb = `BackgroundService` i web-hosten; datouttrekk bak `IDatouttrekk` (default Claude API, lokasjon IT-avklart). Se beslutningsloggen for full fremdrift og åpne forhold.
...
- `/backend` — .NET-solution `Aarshjul.slnx` med lagdelte prosjekter:
  ...
  - `Aarshjul.Web` — ASP.NET Core-host: Blazor-komponenter (visninger, adminflater, internkalender under `Components/Pages/Internkalender`), minimal-API, Entra-auth. Selve app-UI bor her (Interactive Server).
...
- Test (alle): `dotnet test backend/Aarshjul.slnx` — 159 tester (synlighet, datoberegning, innlegging, claims, HTTP-integrasjon, godkjenningskø, brukerforslag, Word-eksport, generering/valgår/anker-kjeder, demo-seed, datouttrekk/opplasting, eksport/import av frister og årsmaler, generering ved duplikat løp).
- Kjør appen lokalt: `dotnet run --project backend/Aarshjul.Web` (krever DB + Entra-konfig; se appsettings).
- **Kjør lokal demo (uten Azure SQL/Entra):** `dotnet run --project backend/Aarshjul.Web --launch-profile demo`, så `/demo` (port 5014). Demo-profilen kjører i miljøet **Development** med konfig-flagget **`Demo=true`** — SQLite (`aarshjul-demo.db`, git-ignorert), `EnsureCreated` + demo-seed (`Demodata`), og cookie-basert dev-innlogging (ingen Entra). **VIKTIG:** demo aktiveres av flagget `Demo=true`, IKKE av `ASPNETCORE_ENVIRONMENT=Demo` sammen med `dotnet run` — `MapStaticAssets` i et ikke-Development-miljø forventer *publiserte* assets, så `ASPNETCORE_ENVIRONMENT=Demo dotnet run` gir 500 på scoped CSS/`blazor.web.js` (helt ustylet side). `ASPNETCORE_ENVIRONMENT=Demo` fungerer kun mot en publisert build. Autorisering/synlighet kjører nøyaktig som i produksjon — kun identitetskilden er byttet. Demo-kode rører ikke produksjonsstien.
...
- **Visning:** `Web/Komponenter/Visningsside.cs` (base for de tre visningene), `Web/Visning/Visningstilstand.cs` (felles filter), `Visningformat.cs`, `Fristkort`/`Filterpanel`, sider i `Components/Pages` (Kalender [`/` + `/kalender`]/Aarshjulvisning + `Admin/Frister`,`RedigerFrist`,`Innsyn`). API: `Web/Api/FristEndepunkter.cs` (`/api/frister`, `/landing`). **MERK:** den separate «Nå»-/landingsvisningen (`Home.razor`) er fjernet 2026-07-22 (forenkling, se beslutningsloggen); `HentLandingsutvalgAsync`/`/api/frister/landing` består og kan gjenbrukes. **`_Imports.razor` i web-roten MÅ ha `@using Microsoft.AspNetCore.Components.Web`** ellers kompilerer direktiv-attributter i `Komponenter/` bokstavelig og interaktivitet/filtre dør.
