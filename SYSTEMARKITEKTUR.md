...
**Utvidelse — auto-versjonsmatching og «foreslått fjernet» (designintervju 2026-06-19).** Ved re-uttrekk av en republisert versjon parres de uttrukne fristene mot de publiserte på en **funksjons-/tittelavledet identitet innen `loep` + `budsjettaar`**, ikke på `dokument_nokkel`/nummer alene (nummeret kan skifte, og ett dokument gir flere frister). En **entydig** match gir et auto-endringsforslag mot den matchede fristen; en **tvetydig eller manglende** match — eller en samlet usikker matching — sendes til **manuell kobling av administrator** framfor et gjettet auto-forslag. Mangler en publisert frist match i et ellers trygt matchet re-uttrekk, tennes et **«foreslått fjernet»**-forslag til administrator: et tredje køutfall ved siden av ny og endret. Også en fjerning er altså et forslag som passerer køen — ingenting fjernes automatisk. Hvordan funksjonsnøkkelen utledes (tittelnormalisering vs. eget funksjonstype-felt) avgjøres ved koding; matchingen må tåle flere frister per dokument.
...
Feltene fra kravdokumentet videreføres: `id` (stabil id fra Entra), `navn`, `funksjonsrolle` (`administrator`, `bidragsyter`, `leser`), `grupper` og `er_fin`. Funksjonsrolle og synlighetsgrupper er adskilte akser.

**Utvidelse — automatisk administrator via Entra-gruppe (Endring 2, 2026-07-30).** Administratorrollen kan nå tildeles automatisk fra Entra: alle i en konfigurert admin-gruppe (i praksis seksjon SBR i Finansavdelingen) blir administrator uten å måtte utpekes manuelt. Dette forenkler overgangen når en administrator slutter, og reverserer den opprinnelige regelen om at admin kun utpekes av en sittende administrator (kravdok. 2.2 / beslutning 2026-06-18). Gruppen(e) angis konfigurerbart (`Administratortilgang`-seksjonen; IT setter SBRs Entra-objekt-id-er). Tilgangen er **medlemskapsstyrt**: er brukeren i admin-gruppen blir vedkommende administrator ved innlogging, forlater brukeren gruppen mister en *automatisk* tildelt administrator rollen ved neste innlogging. Modellen bærer derfor et felt `admin_via_entra` (`AdminViaEntra`) som skiller en gruppestyrt administrator fra en **seedet/manuelt satt** administrator; sistnevnte (`admin_via_entra = false`) beholdes som nødvei og nedgraderes aldri automatisk. `POL` og synlighetsgrupper settes fortsatt aldri automatisk til admin-status — dette gjelder kun funksjonsrollen.
...
Datouttrekket bruker språkmodell til tolkning der formuleringene varierer, og leverer det strukturerte per-felt-resultatet beskrevet i 3.2 — tolket verdi, kildespenn og konfidens per felt. Uttrekket ligger bak et eget grensesnitt (`IDatouttrekk`, samme prinsipp som kildeleddet), slik at modellvalg og kjørelokasjon (ekstern Claude API vs. Azure-vertet) er et utbyttbart, IT-avklart ledd og ikke en innebygd binding (designintervju 2026-06-19). Der en språkmodell ennå ikke er koblet på, brukes en **deterministisk offline-implementasjon** av `IDatouttrekk` (regelbasert datogjenkjenning + løpsklassifisering) bak samme grensesnitt — slik at uttrekk virker i demo og uten IT-avklaring, og skjerpes til modellbasert uttrekk uten annen ombygging. Relative frister i kilden («ultimo mars») mappes til gjentaksregler framfor harde datoer. Alt uttrekk er forslag.

**Manuell opplasting som inntaksvei (permanent funksjon).** Administrator kan laste opp et dokument (PDF) manuelt. Opplasting mater dokumentet inn i det **samme** uttrekks- og køleddet som den automatiske innhentingen — den hopper bare over kildeleddets `oppdag()`/`hent()`. Teksten trekkes ut (PDF-tekstuttrekk er et eget, byttbart ledd), kjøres gjennom `IDatouttrekk` og totrinns-klassifiseringen (5), og blir `Forslag` (`opphav = robot`, kilde «Opplastet: «filnavn»») med per-felt `UttrekksBevis` og de samme usikkerhetsflaggene. Opplastede dokumenter registreres i behandlet-dokument-registeret (3.4, kilde «opplastet», dedup på filnavn + innholdshash). **Opplasting er en administratorfunksjon**; bidragsytere sender kun egne forslag via skjema (BRUKERHISTORIER 3.1), aldri via dokumentopplasting. Ingenting publiseres uten godkjenning — prinsippet gjelder uendret for både automatisk og opplastet uttrekk.
...
Robotkort viser tolkede felter, tekstutdrag, kildelenke og per-felt usikkerhetsflagg (3.2). Brukerkort viser innsenders navn fra innlogget identitet og foreslått synlighet. Endringsforslag vises med en før/etter-fremstilling mot den berørte fristen (3.2). Et **«foreslått fjernet»**-kort (3.4) viser hvilken publisert frist som mangler match og foreslås avpublisert.

Handlingene er godkjenn, juster (som åpner redigeringsskjemaet) og avvis, samt vurder for «ukjent type». Ved avvisning bevares forslaget med status `avvist`, og et varsel opprettes for innsenderen ved brukerforslag (3.7). For et **endringsforslag** velges ingen synlighet i køen: godkjenning oppdaterer kun fristens innhold, mens `synlig_for` står urørt (3.2, punkt C). Godkjenning av et **«foreslått fjernet»**-kort avpubliserer den berørte fristen; avvisning lar fristen stå uendret.
...
**Delbar kalender-feed / abonnementslenke (Endring 1, 2026-07-30).** En engangs-`.ics`-import er et øyeblikksbilde og oppdaterer seg ikke; for en **delt kalender som holder seg oppdatert** (SharePoint/Outlook) trengs en abonnementslenke klienten poller. Modellen `Kalenderabonnement` (`Token`, `GruppeKode` (null = «alt»), `Etikett`, `Aktiv`) forvaltes av `IKalenderabonnement`. Administrator oppretter én lenke **per synlighetsgruppe** (POL, FA, FAG, …) eller for «alt» på flaten `/admin/abonnement`, og kan skru en lenke av/på (tilbakekalle) eller slette den. Feed-endepunktet `GET /kalender/feed/{token}.ics` er **anonymt**: kalenderklienter kan ikke logge inn interaktivt, så det uknekkbare tokenet (256 bit) i URL-en *er* autoriseringen. Endepunktet gjenbruker `IcsEksportTjeneste`, og — avgjørende — bygger synlighetskonteksten fra lenkens gruppe (`Synlighetskontekst.ForGruppe`/`SerAlt`), slik at **all filtrering fortsatt skjer på server**: en delt POL-lenke gir kun POL-settet, aldri FIN-interne frister. Ukjent eller avskrudd token gir 404 (ingen enumerering). Dette er et bevisst, avgrenset unntak fra «alt bak innlogging» — synligheten håndheves per gruppe på server, og en lekket lenke kan tilbakekalles. Feeden er framoverskuende (ingen historikk) uten periodegrense, så den speiler «kalenderen slik den er nå». Den manuelle nedlastingen (over) beholdes ved siden av. En abonnementslenke virker først live når appen er nådd over internett (Azure-utrulling).

**Åpent forhold — identitetssperre på feeden (avgjøres ved Azure-utrulling).** En abonnementsfeed kan ikke EntraID-sjekke den som ser kalenderen: kalenderklienter (og ofte Microsofts M365-servere) henter URL-en anonymt, uten pålogging. Feeden er derfor sikret med hemmelig token + server-side gruppefiltrering + tilbakekalling, ikke med identitet. Vil man at *kun personer med EntraID i departementene* skal se innholdet (slik at en lekket lenke ikke viser noe), er den eneste veien som også beholder auto-oppdatering **Microsoft 365-native deling via Graph** (skriv fristene inn i en delt M365-/gruppekalender som Microsoft tilgangsstyrer). Alternativt herdes feeden (utløp, per-mottaker-token, hentelogg) eller sensitive grupper (POL/FIN-internt) sperres fra feed. Valget tas når Entra-/M365-oppsettet er avklart med IT; se beslutningsloggen 2026-07-30 «ÅPENT FORHOLD: tilgangssikring av kalender-feeden».
...
**Årsmaler (gjentaksregler) eksporteres/importeres tilsvarende** (2026-07-22) via `IMalDatautveksling`/`MalDatautveksling` og endepunktet `/api/eksport/maler-json`, med samme erstatt-alt-semantikk. `Gjentaksregel` har ingen fremmednøkkel fra `Frist` (kun en `GjentaRegelId`-verdi), så erstatt-alt er trygt. Regler med parametre som ikke lar seg tolke for regeltypen, hoppes over ved import med en advarsel, slik at en senere genereringskjøring ikke feiler på en ugyldig regel. `/admin/data` har én seksjon per datatype (frister og årsmaler).

Malregelskjemaet (`RedigerMalregel`) lar administrator koble en `relativ_til_milepael`-regel til en **eksisterende regel via en nedtrekksliste** framfor å skrive anker-løpet i JSON; verktøyet bygger `{"anker_loep":…,"offset_dager":…}` automatisk. Ankeret er fortsatt et løp, og ved dupliserte løp løses det til representanten (første regel for løpet), jf. genereringslogikken (8).
...
## 12. Internkalender for SBR (2026-08-03)

En SBR-intern arbeidsliste, arkitektonisk atskilt fra fristene og den gruppebaserte synligheten. Hele funksjonen gates av en egen policy `ErSbr` (i v1 = `Administrator`, siden admin gis via Entra-gruppen SBR, jf. 3.5), lagt ved siden av de øvrige policyene slik at SBR senere kan skilles fra admin uten å endre hver flate. Internkalender-data er SBR-internt og går aldri gjennom den synlighetsfiltrerte leseflaten; `synlig_for`-mekanismen (4) berøres ikke.

**Datamodell (egne tabeller, EF-migrasjon `Internkalender`).**

- `InternRunde` — en konkret oppgaveplan for én runde: `Rundetype` (Marsrunden/Augustrunden/RNB/Nysaldering/Regnskap/Øvrig), `Aar` (budsjett-/regnskapsår; null for Øvrig), opprettet-info og `SistSynkronisert`. En rundetype + år er unik.
- `InterntGjoeremaal` — et gjøremål i en runde: `Tittel` (eneste påkrevde felt — «mangelfulle gjøremål» tillates), `Notat`, tidfesting (`Tidfestingstype` ∈ Ingen/KonkretDato/TentativMaaned/AnkerRelativ/Rundeposisjon med tilhørende felter), avledet `Sorteringsdag` (null = udatert, sorteres sist) og `VenterPaaAnker`, status (Aktiv/Fullfoert) med `FullfoertAvId`/`FullfoertAvNavn`/`FullfoertTid`, samt `Opphav` (Manuell/Generert), `GenerertFraRegelId` og `ManueltEndret` (for synk). `GjoeremaalAnsvarlig` (bruker-ref eller fritekst) er en barnkolleksjon; «kun mine» matcher på bruker-id.
- `GjoeremaalRegel` — en generell, årsuavhengig regel (mal): tittel, notat, tidfesting (måned/dag + `AarforskyvningJustering`, tentativ måned, anker-løp, eller rundeposisjon), knyttet til én eller flere rundetyper (`RegelRundetype`) og med standard ansvarlige (`RegelAnsvarlig`).

**Runder som domenelogikk.** `Runder` (Domain) definerer hver rundetypes månedsspenn og kalenderår-forskyvning (Mars/August = t-1, RNB/Nysaldering = t, Regnskap/Øvrig = 0), og avleder en dato for en rundeposisjon. En regels tidfesting oversettes til en konkret dato for rundeåret ved generering; anker-løp løses mot en publisert, godkjent frist på samme løp der budsjett-/regnskapsåret er rundens år — finnes ingen, blir gjøremålet «venter på ankerdato». Sorteringsdagen gjenbruker `Datoberegning.Sorteringsdag`/virkedagjustering.

**Generelt → konkret.** Generering (`InternkalenderTjeneste.GenererRundeAsync`) leser reglene som gjelder rundetypen og lager ett gjøremål per regel (`Opphav = Generert`, `GenerertFraRegelId` satt). Den konkrete runden er et **snapshot**; senere endringer i reglene slår ikke tilbake automatisk. **Synkronisering** (`ForberedSynkAsync`/`SynkroniserAsync`) bygger forslag (legg til / oppdater / fjern) som administrator godtar enkeltvis; forslag genereres aldri for — og synk rører aldri — gjøremål som er huket av (`Fullfoert`) eller manuelt endret (`ManueltEndret`). Dette speiler «foreslått fjernet»-tankegangen (3.4): ingenting fjernes eller overskrives automatisk.

**Tjenestelag.** `IInternkalender`/`InternkalenderTjeneste` (runder, gjøremål, avhuking, «mine» på tvers av runder, generering, synk) og `IGjoeremaalRegler`/`GjoeremaalRegelTjeneste` (regel-CRUD) i Application/Infrastructure. Flater under `Components/Pages/Internkalender` (oversikt, rundevisning med aktiv/ferdig + hurtiginnlegging + personfilter, gjøremål-skjema, «Mine gjøremål», regler, synk), alle bak `ErSbr`. Avgrensning v1: ingen eksport/Outlook/varsler.
